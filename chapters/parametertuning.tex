...
Looking to the parameters, how does they correspond to other values found in the literature? First $\hat{\alpha} $ is the constant in the wage equation, since the Mincer equation is log transformed a reversal an interpretation requires the $\exp (\cdot)$ transformation. Assuming no human capital, $G=0$, yields $\exp (\hat{\alpha}) = \exp ( 4.609 ) = 100$. Or in other words, a totally inexperience worker would in this model have an hourly wage of 100 DKK (not far from the actual minimum wage). 

Looking to $\delta$ the human capital depreciation I find a value of appoximately $20 \%$ pr. year. Compared to other studies (Kunze 2002 XXX) she find that parental leave reduces wages for women with about $13$ to $18 \%$ pr year, while other work interuptions is about $2$  to $5 \%$ a year. (Light & ureta 1995) finds values for human capital depreciation at about 13 $\%$ a year.
